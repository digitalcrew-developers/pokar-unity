...
public class CareerManagerTeen : MonoBehaviour
{
    public static CareerManagerTeen instance;
    public int selectedIndex_CareerMenuScreen = 0;

    //DEV_CODE
    public GameObject requestObj, dataObj;
    public List<RequestDataTeen> requestDataList = new List<RequestDataTeen>();
    ...
    public void OnServerResponseFound(RequestType requestType, string serverResponse, bool isShowErrorMessage, string errorMessage)
    {
        ...
        if (requestType == RequestType.UpdateMultiAccountRequestStatus)
        {
            ...
            {
                MainMenuControllerTeen.instance.ShowMessage("Unable to update request..");
            }
        }
            ...
            if (data["status"].Equals(true))
            {
                if (data["getData"].Count > 0)
                {
                    ...
                    for (int i = 0; i < data["getData"].Count; i++)
                    {
                        ...
                        RequestDataTeen reqData = new RequestDataTeen();
