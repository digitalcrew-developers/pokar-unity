...
namespace Com.TheFallenGames.OSA.Demos.GridWithCategories
{
	...
	/// have some dummy cells inserted here and there (more info can be found below and in <see cref="GridWithCategoriesDataUtil"/>)</para>
	/// </summary>
	...
	{
		// It's very useful to store a reference to the category in the cell itself, because then you can easily retrieve it
	...
	public class MyCellViewsHolder : CellViewsHolder
	{
		...
		public void UpdateViews(CellModel model, MyGridParams parameters)
		{
			switch (model.type)
			{
				...
				case CellModel.CellType.FOR_ROW_COMPLETION:
				// Element inside an empty (separator) row
				case CellModel.CellType.IN_ROW_SEPARATING_CATEGORIES:
					SetVisible(false);
