...
public class TournamentInGameUiManager : MonoBehaviour
{
    public static TournamentInGameUiManager instance;
    ...
    private List<TournamentInGameActiveScreens> inGameActiveScreens = new List<TournamentInGameActiveScreens>();
    ...
    public void OnServerResponseFound(RequestType requestType, string serverResponse, bool isShowErrorMessage, string errorMessage)
    {
        ...
        if (requestType == RequestType.deductFromWallet)
        {
            ...
            if (data["success"].ToString() == "1")
            {
                ...
                TournamentSocketController.instance.SendWinningBooster(int.Parse(tableId), winnigBoosterAmount, winningBoosterCardName);
            }
            else
            {
    ...
    public void OnSpinWheelArrowBtnClick()
    {
        ...
        ShowScreen(TournamentInGameScreens.SpinWheelScreen);
    }
    ...
    public void OnSpwanTipCoin()
    {
        ...
        TournamentSocketController.instance.TipToDealer();
    ...
    {
        Debug.Log(eventName + " " + useRaisePotWise + ", " + TournamentInGameManager.instance.GetPotAmount()); 
        ...
        switch (eventName)
        {
            case "rabbit":
                {
                    TournamentInGameManager.instance.RabbitButton.SetActive(false);
                    StartCoroutine(TournamentInGameManager.instance.WaitAndShowRabbit());
                }
                break;
            case "store":
                {
                    ShowScreen(TournamentInGameScreens.InGameShop);
                }
                break;
            ...
            case "menu":
                {
                    ShowScreen(TournamentInGameScreens.Menu);
                }
                break;

            case "missions":
                {
                    ShowScreen(TournamentInGameScreens.Missions);
                }
                break;
            case "emojiScreen":
                {
                    if (TournamentInGameManager.instance.AmISpectator)
                        return;
                    ShowScreen(TournamentInGameScreens.EmojiScreen);
                }
                break;
            case "fold":
                {
                    PlayerScript player = TournamentInGameManager.instance.GetMyPlayerObject();
                    player.ResetTurn();
                    TournamentInGameManager.instance.OnPlayerActionCompleted(PlayerAction.Fold, 0, "Fold");
                }
                break;
            ...
            case "call":
                {
                    ...
                    TournamentInGameManager.instance.OnPlayerActionCompleted(PlayerAction.Call, (int)availableCallAmount, "Call");
                }
                break;
            case "allin":
                {
                    PlayerScript player = TournamentInGameManager.instance.GetMyPlayerObject();
                    player.ResetTurn();
                    TournamentInGameManager.instance.OnPlayerActionCompleted(PlayerAction.AllIn, (int)player.GetPlayerData().balance, "AllIn");
                }
                break;
            ...
            case "check":
                {
                    ...
                    TournamentInGameManager.instance.OnPlayerActionCompleted(PlayerAction.Check, 0, "Check");
                }
                break;
            ...
            case "raiseOpen":
                {
                    ...
                    if (player != null)
                    {
                        ...
                        ToggleRaisePopUp(true, availableCallAmount, player.GetPlayerData().balance, TournamentInGameManager.instance.GetPotAmount());
                    }
                    else
                    {
#if ERROR_LOG
            ...
            case "raiseDone":
                {
                    if (sliderText.text == "All In")
                    {
                        ...
                        {
                            player.ResetTurn();
                            TournamentInGameManager.instance.OnPlayerActionCompleted(PlayerAction.Raise, (int)player.GetPlayerData().balance, "AllIn");
                        }
                        else
                        {
#if ERROR_LOG
                    ...
                    {
                        //OnSliderValueChange();
                        TournamentInGameManager.instance.OnPlayerActionCompleted(PlayerAction.Raise, (int)selectedRaiseAmount, "Raise");
                    }
                }
                break;
            ...
            case "pot":
                {
                    ...
                    {
                        slider.value = TournamentInGameManager.instance.GetPotAmount();
                        OnSliderValueChange();
                    }
            ...
            case "halfPot":
                {
                    if (useRaisePotWise) // PotWise Calculation
                    {
                        slider.value = (TournamentInGameManager.instance.GetPotAmount() / 2f);
                        OnSliderValueChange();
                    }
                    else // Call Amount wise calculation
                    {
            ...
            case "thirdPot":
                {
                    if (useRaisePotWise) // PotWise Calculation
                    {
                        slider.value = ((TournamentInGameManager.instance.GetPotAmount() * 2f) / 3f);
                        OnSliderValueChange();
                    }
                    else // Call Amount wise calculation
                    {
            ...
            case "handHistory":
                {
                    ShowScreen(TournamentInGameScreens.HandHistory);
                }
                break;

            case "realTimeResult":
                {
                    ShowScreen(TournamentInGameScreens.RealTimeResult);
                }
                break;
            case "pointEarnopen":
                {
                    ShowScreen(TournamentInGameScreens.PointEarnMsg);
                }
                break;
            ...
            case "chat":
                {
                    ...
                    {
                        ShowScreen(TournamentInGameScreens.Chat);
                    }
                    else
                    {
    ...
    public void ShowScreen(TournamentInGameScreens screenName, object[] parameter = null)
    {
        ...
        if (!IsScreenActive(screenName))
        {
            ...
            switch (screenName)
            {
                ...
                case TournamentInGameScreens.HandHistory:
                    ...
                    break;
                case TournamentInGameScreens.PointEarnMsg:
                    {
    ...
    {
        if (!IsScreenActive(TournamentInGameScreens.Message))
        {
            TournamentInGameActiveScreens mainMenuScreen = new TournamentInGameActiveScreens();
            mainMenuScreen.screenName = TournamentInGameScreens.Message;
            mainMenuScreen.screenLayer = GetScreenLayer(TournamentInGameScreens.Message);

            GameObject gm = Instantiate(screens[(int)TournamentInGameScreens.Message], screenLayers[(int)mainMenuScreen.screenLayer]) as GameObject;
            mainMenuScreen.screenObject = gm;
    ...
    public void DestroyScreen(TournamentInGameScreens screenName)
    {
    ...
    private bool IsScreenActive(TournamentInGameScreens screenName)
    {
        for (int i = 0; i < inGameActiveScreens.Count; i++)
        {
            if (inGameActiveScreens[i].screenName == screenName)
    ...
    private ScreenLayer GetScreenLayer(TournamentInGameScreens screenName)
    {
        switch (screenName)
        {
            ...
            case TournamentInGameScreens.Loading:
...
public class TournamentInGameActiveScreens
{
    public GameObject screenObject;
    public TournamentInGameScreens screenName;
    public ScreenLayer screenLayer;
}

public enum TournamentInGameScreens
{
    Message,
    Loading,
    Reconnecting,
