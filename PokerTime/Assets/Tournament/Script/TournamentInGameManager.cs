...
public class TournamentInGameManager : MonoBehaviour
{
    public static TournamentInGameManager instance;
    ...
    {
        instance = this;
        //TournamentInGameManager.instance = null;
    ...
    private void Start()
    {
        ...
        //DEV_CODE
        videoHeight = (int)TournamentInGameUiManager.instance.height;
        videoWidth = (int)TournamentInGameUiManager.instance.width;
    ...
    public void OnRabbitDataFound(string responseText)
    {
        ...
        TournamentInGameUiManager.instance.ToggleSuggestionButton(false);
        TournamentInGameUiManager.instance.ToggleActionButton(false);
        int vipCard = 0;
    ...
    private IEnumerator WaitAndShowCardAnimation(PlayerScript[] players, PlayerScript playerScriptWhosTurn)
    {
        ...
        if (isGameStart)
            TournamentSocketController.instance.SetSocketState(SocketState.Game_Running);
    ...
    public IEnumerator SwitchTables()
    {
        ...
        TournamentSocketController.instance.SendSwitchTable();
    ...
    private void SwitchTurn(PlayerScript playerScript, bool isCheckAvailable)
    {
        ...
        if (currentPlayer.IsMe())
        {
            ...
            SuggestionActions selectedSuggestionAction = TournamentInGameUiManager.instance.GetSelectedSuggestionAction();
            TournamentInGameUiManager.instance.ResetSuggetionAction();
                    ...
                    case SuggestionActions.Call_Any:
                        {
                            ...
                            else
                            {
                                ...
                                TournamentInGameUiManager.instance.ToggleActionButton(true, currentPlayer, isCheckAvailable, LAST_BET_AMOUNT);
                            }
                        }
                        break;
        ...
        else
        {
            ...
            {
                int callAmount = GetLastBetAmount() - (int)GetMyPlayerObject().GetPlayerData().totalBet;
                TournamentInGameUiManager.instance.ToggleSuggestionButton(true, isCheckAvailable, callAmount, GetMyPlayerObject().GetPlayerData().balance);
            }
        }
    }
    ...
    public IEnumerator SwitchToAnotherTableReset()
    {
        ...
        TournamentSocketController.instance.SendLeaveMatchRequest();
        yield return new WaitForSeconds(GameConstants.BUFFER_TIME);
        TournamentSocketController.instance.ResetConnection();
    }
    ...
    public void OnClickStandupBtn()
    {
        isSeatRotation = false;
        AmISpectator = true;
        TournamentSocketController.instance.SendStandUpdata();
    }
    ...
    public void OnPlayerActionCompleted(PlayerAction actionType, int betAmount, string playerAction)
    {
        ...
        if (actionType == PlayerAction.Fold)
        {
            ...
            TournamentSocketController.instance.SendFoldRequest(GetMyPlayerObject().GetLocalBetAmount());
        }
        else
        {
            ...
            TournamentSocketController.instance.SendBetData(betAmount, GetMyPlayerObject().GetLocalBetAmount(), playerAction, GetMatchRound());
        }
    }
        ...
        if (remainingTime > 1)
        {
           ...
           // InGameUiManager.instance.LoadingImage.SetActive(true);
            if (!isRematchRequestSent)
            {
                ...
                {
                    //DEV_CODE
                    TournamentInGameUiManager.instance.isSelectedWinningBooster = false;
                    ...
                    if (isTopUpDone || availableBalance >= GlobalGameManager.instance.GetRoomData().minBuyIn)
                    {
                        ...
                        TournamentSocketController.instance.SendReMatchRequest("Yes", "0");
                    }
                    else
                    {
                        ...
                        if (userMainBalance < EPSILON)
                        {
                            ...
                            TournamentSocketController.instance.SendTopUpRequest(balanceToAdd);
                        ...
                        else
                        {
                            ...
                            {
                                TournamentInGameUiManager.instance.ShowMessage("You don't have enough coins to play, please purchase some coins to continue");
                                // TODO call sit out
        ...
        if (TournamentSocketController.instance.GetSocketState() == SocketState.Game_Running)
        {
            ...
            if (currentPlayer != null)
            {
                ...
                if (currentPlayer.IsMe())
                {
                    ...
                    {
                        TournamentInGameUiManager.instance.ToggleActionButton(true, currentPlayer, isMyTurn, LAST_BET_AMOUNT, GetMyPlayerObject().GetPlayerData().balance);
                ...
                {
                    TournamentInGameUiManager.instance.raisePopUp.SetActive(false);
                    currentPlayer.ShowRemainingTime(remainingTime);
                    //currentPlayer.StartPlayerTimer(remainingTime);
                }
            }
...
//InGameUiManager.instance.LoadingImage.SetActive(true);
    ...
    public void OnPlayerObjectFound(string serverResponse)
    {
        ...
        if (!TournamentInGameUiManager.instance.isSelectedWinningBooster)
        {
        ...
        if (data[0].Count > 0)
        {
            ...
            if (data[0].Count == 1)
            {
                ...
                TournamentSocketController.instance.SetSocketState(SocketState.WaitingForOpponent);
            ...
            if (TournamentSocketController.instance.GetSocketState() == SocketState.WaitingForOpponent)
            {
                TournamentSocketController.instance.SetTableId(data[0][0]["tableId"].ToString());
                ...
                //if (isMatchStarted) // Match is started
                {
                    ...
                    for (int i = 0; i < data[0].Count; i++)
                    {
                        ...
                        TournamentInGameUiManager.instance.tableId = data[0][i]["tableId"].ToString();
            ...
            else if (TournamentSocketController.instance.GetSocketState() == SocketState.Game_Running)
            {
