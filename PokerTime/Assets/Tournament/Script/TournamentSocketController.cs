...
{
    public static TournamentSocketController instance;
    ...
    private void ReConnect()
    {
        ...
        socketManager = new SocketManager(new Uri(GameConstants.TOURNAMENT_SOCKET_URL + "/socket.io/"), socketOptions);
//        Debug.LogError("URL IS " + GameConstants.SOCKET_URL + "/socket.io/");
    ...
    private void PlayerExit(Socket socket, Packet packet, object[] args)
    {
        ...
        TournamentInGameManager.instance.gameExitCalled = true;
        ResetConnection();
    }
        ...
        if (socketResponse.Count > 0)
        {
            ...
            switch (responseObject.eventType)
            {
                ...
                case SocketEvetns.PLAYER_OBJECT:
                    if (TournamentInGameManager.instance != null)
                    {
                ...
                case SocketEvetns.ON_OPEN_CARD_DATA_FOUND:
                    TournamentInGameManager.instance.OnOpenCardsDataFound(responseObject.data);
                    break;
                case SocketEvetns.RABBIT_CARDS:
                    TournamentInGameManager.instance.OnRabbitDataFound(responseObject.data);
                    break;
                ...
                case SocketEvetns.ON_NEXT_ROUND_TIMER_FOUND:
                    TournamentInGameManager.instance.OnNextMatchCountDownFound(responseObject.data);
                    break;

                case SocketEvetns.ON_GAME_OVER_TIMER_FOUND:
                    TournamentInGameManager.instance.OnGameOverCountDownFound(responseObject.data);
                    break;

                case SocketEvetns.ON_CALL_TIMER_FOUND:
                    TournamentInGameManager.instance.OnTurnCountDownFound(responseObject.data);
                    break;


                case SocketEvetns.ON_GAME_START_TIMER_FOUND:
                    TournamentInGameManager.instance.OnGameStartTimeFound(responseObject.data);
                    break;

                case SocketEvetns.ON_ROUND_NO_FOUND:
                    // InGameUiManager.instance.LoadingImage.SetActive(false);
                    TournamentInGameManager.instance.OnRoundDataFound(responseObject.data);
                    break;
                ...
                case SocketEvetns.ON_RESULT_FOUND:
                    TournamentInGameManager.instance.OnResultResponseFound(responseObject.data);
                    break;
                ...
                case SocketEvetns.ON_BET_DATA_FOUND:
                    TournamentInGameManager.instance.PlayerTimerReset();
                    TournamentInGameManager.instance.OnBetDataFound(responseObject.data);
                    break;
                ...
                case SocketEvetns.ON_RECONNECTED:
                    if (!responseObject.data.Contains("Reconnect Success"))
                    {
                        ...
                        TournamentInGameUiManager.instance.ShowMessage("Match Data not found", () =>
                        {
                ...
                case SocketEvetns.ON_PlayerStandUp:
                    TournamentInGameManager.instance.StandUpPlayer(responseObject.data);
                    break;
                case SocketEvetns.ON_SendEmoji:
                    TournamentInGameManager.instance.SendEmoji(responseObject.data);
                    break;
                case SocketEvetns.ON_ALL_TIP_DATA:
                    TournamentInGameManager.instance.TipToDealer(responseObject.data);
                    break;
                case SocketEvetns.ON_POINT_UPDATE:
                    TournamentInGameManager.instance.PointUpdated(responseObject.data);
                    break;
                case SocketEvetns.ON_GET_RANDOM_CARD:
                    ...
                    TournamentInGameUiManager.instance.winningBoosterCardName = cardDt.cardNumber.ToString();
                    TournamentInGameUiManager.instance.arrowPopUpText.text += TournamentInGameUiManager.instance.winningBoosterCardName;

                    //Debug.Log("Name: " + cardDt.cardNumber);
                    break;
                case SocketEvetns.ON_UPDATETOURNAMENTLIST:
                    TournamentInGameUiManager.instance.ShowScreen(TournamentInGameScreens.TournamentLobby);
...
#if UNITY_EDITOR
        if (GlobalGameManager.instance.CanDebugThis(SocketEvetns.ON_UPDATETOURNAMENTLIST))
        {
            //Debug.Log("OnGetTournamentList = " + responseText + "  Time = " + System.DateTime.Now);
...
#endif
        SocketResponse response = new SocketResponse();
        response.eventType = SocketEvetns.ON_UPDATETOURNAMENTLIST;
        response.data = responseText;
        socketResponse.Add(response);
    }
    ...
    void OnNextRoundTimerFound(Socket socket, Packet packet, params object[] args)
    {
        ...
        //DEV_CODE
        if (TournamentInGameUiManager.instance.actionButtonParent.activeSelf)
            TournamentInGameUiManager.instance.actionButtonParent.SetActive(false);
        
        if (TournamentInGameUiManager.instance.suggestionButtonParent.activeSelf)
            TournamentInGameUiManager.instance.suggestionButtonParent.SetActive(false);
        //***********************************************
    ...
    {
        string responseText = JsonMapper.ToJson(args);
        TournamentInGameManager.instance.Pot.SetActive(false);
        TournamentInGameManager.instance.DeactivateAllPots();
#if DEBUG
    ...
    public void GetTournamentList()
    {
        ...
        SocketRequest request = new SocketRequest();
        request.emitEvent = "getTournamentList";

        Debug.LogError(/*gameObject.transform.parent.name + */"getTournamentList: " + requestStringData);
    ...
    public void SendFoldRequest(int totalBetInRound)
    {
        ...
        requestData.userData.playerAction = TournamentInGameManager.instance.GetLastPlayerAction();
        requestData.userData.roundNo = TournamentInGameManager.instance.GetMatchRound();
    ...
    public void ResetConnection(bool isReconnecting = false)
    {
        ...
        if (TournamentInGameManager.instance.gameExitCalled)
...
#region ReconnectProtocols
    ...
    private IEnumerator WaitForReconnect()
    {
        ...
        TournamentInGameUiManager.instance.ShowScreen(TournamentInGameScreens.Reconnecting);
        ...
        {
            TournamentInGameUiManager.instance.DestroyScreen(TournamentInGameScreens.Reconnecting);
            TournamentInGameUiManager.instance.ShowMessage("Connection error, check your network connection and try again.", () =>
            {
                StartCoroutine(WaitForReconnect());
            },
